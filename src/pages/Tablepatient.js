...
function Tablepatient() {
  ...
  useEffect(() => {
    ...
    axios
      .get("http://10.10.50.24:3019/api/getPatient", config)
      .then((res) => {
        setdata(res.data);
      })
      .catch(() => {
  ...
  const handleDelete = async (id) => {
    await axios
      .delete(`http://10.10.50.24:3019/api/deletePatient/${id}`, config)
  ...
  const handleSearch = async () => {
    ...
    let resultat = await fetch(
      `http://10.10.50.24:3019/api/searchPatient/${key ? key : "all"}`,
      config
    );
